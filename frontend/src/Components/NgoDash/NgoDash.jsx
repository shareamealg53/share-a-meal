...
export default function NgoDash() {
    ...
    const fetchNGOData = async () => {
      try {
        ...
        const [mealsRes, claimsRes] = await Promise.all([
          apiRequest("/meals", { headers: { Authorization: `Bearer ${token}` } }),
          apiRequest("/claims/my", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
        ...
        const ngoData = {
          ...
          stats: {
            availableNearby: mealsRes.meals?.filter(m => m.status === "AVAILABLE").length || 0,
            reservedMeals: claimsRes.claims?.filter(c => c.status === "CLAIMED").length || 0,
            totalMealsSaved: claimsRes.claims?.length || 0,
          },
          recommendations: mealsRes.meals?.slice(0, 5).map(meal => ({
            id: meal.id,
            title: meal.title,
  ...
  if (loading) return <div className={styles.dashboard}>Loading dashboard...</div>;
  if (error) return <div className={styles.dashboard}><p style={{ color: "red" }}>Error: {error}</p></div>;
